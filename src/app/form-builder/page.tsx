...
import type { FormConfig, FormField, FieldType } from '@/lib/types';
import { generateId, createField, getDefaultFormConfig, slugify } from '@/lib/utils';
import { generateHTML, generateCSS } from '@/lib/code-generator';
...
export default function FormBuilderPage() {
  ...
  const softwareSchema = JSON.stringify({
    ...
    name: 'FormForge - Free Online Form Builder',
    ...
    <div className="bg-gray-50 min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500">Drag, drop, and customize your form fields</p>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div className={`lg:col-span-2 ${tab !== 'build' ? 'hidden lg:block' : ''}`}>
            <div className="bg-white rounded-xl border border-gray-200 p-4 sticky top-20">
              ...
              <div className="space-y-1">
                {fieldTypes.map(ft => (
                  <button key={ft.type} onClick={() => addField(ft.type)} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-gray-700 hover:bg-purple-50 hover:text-[--color-primary] rounded-lg transition-colors text-left">
        ...
        <div className="mt-12 bg-gradient-to-r from-purple-50 to-purple-100 rounded-2xl p-8 text-center">
          <h2 className="text-2xl font-bold text-gray-900">Need Hosted Forms with Submissions?</h2>
          <p className="mt-3 text-gray-600 max-w-xl mx-auto">FormForge generates HTML code. If you need hosted forms with response collection and analytics:</p>
          <div className="mt-6 flex flex-wrap justify-center gap-4">
            <a href="/go/typeform" target="_blank" rel="noopener noreferrer nofollow" className="px-6 py-2.5 bg-[--color-primary] text-white rounded-lg font-semibold text-sm hover:bg-[--color-primary-dark]">Try Typeform →</a>
            <a href="/go/jotform" target="_blank" rel="noopener noreferrer nofollow" className="px-6 py-2.5 bg-white text-[--color-primary] border-2 border-[--color-primary] rounded-lg font-semibold text-sm hover:bg-purple-50">Try JotForm →</a>
            <a href="/go/tally" target="_blank" rel="noopener noreferrer nofollow" className="px-6 py-2.5 bg-white text-[--color-primary] border-2 border-[--color-primary] rounded-lg font-semibold text-sm hover:bg-purple-50">Try Tally →</a>
          </div>
        </div>
      </div>
    </div>
